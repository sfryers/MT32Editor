...
namespace MT32Edit
{
    public partial class FormSystemSettings : Form
    {
        ...
        {
            SetMidiChannels();
            if (sendSysEx) MT32SysEx.SendText("Channels 2-10");
        }

        private void radioButtonChannels1to8_CheckedChanged(object sender, EventArgs e)
        {
            SetMidiChannels();
            if (sendSysEx) MT32SysEx.SendText("Channels 1-8 & 10");
        }

        private void SetMidiChannels()
        {
            if (radioButtonChannels2to9.Checked) system.SetMidiChannels2to9();
            else if (radioButtonChannels1to8.Checked) system.SetMidiChannels1to8();
            SetSystemControls();
        }

        private void SetRadioButtons()
        {
            if (system.MidiChannelsAreSet1to8()) radioButtonChannels1to8.Checked = true;
            else if (system.MidiChannelsAreSet2to9()) radioButtonChannels2to9.Checked = true;
            else radioButtonChannelCustom.Checked = true;
        }
